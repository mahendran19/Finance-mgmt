...
from .forms import FormIncome1,FormExpense1
...
def home(request):
    ...
    a=Income1()
    incomes=Income1.objects.filter(user_id=current_user.id)
    b=[a.salary for a in incomes]
    print(sum(b))
   
    
    if Income1.objects.all():
...
def adding(request):
    ...
    if request.method=='POST':
        form= FormIncome1(request.POST or None)
...
def formoney(request):
    ...
    if request.method=='POST':
        form= FormExpense1(request.POST or None)
